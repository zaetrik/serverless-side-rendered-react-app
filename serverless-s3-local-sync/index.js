/**
 * NOT MY CODE; JUST MADE SOME SMALL ADJUSTMENTS
 * Taken from https://github.com/mbudm/serverless-s3-local-sync
 */
...
function cleanFilePathOfLocalDir(filePath, localDir, bucketPrefix, followSymlinks) {
  const formattedLocalDir = localDir
    .split('./')
    .join('')
    .split('/')
    .filter((x) => '' !== x)
  ...
  const formattedBucketPrefix = bucketPrefix
    .split('./')
    .join('')
    .split('/')
    .filter((x) => '' !== x)
  ...
  // used with option followSymlinks
  const localFilePath = filePath.split(formattedLocalDir).pop()
  return `${formattedBucketPrefix}${followSymlinks ? localFilePath : fileName}`
}

function uploadFile(filePath, bucketName, localDir, bucketPrefix = '', followSymlinks = true, client, cli) {
  return new Promise((resolve, reject) => {
    const params = {
      Key: cleanFilePathOfLocalDir(filePath, localDir, bucketPrefix, followSymlinks),
      Bucket: bucketName,
...
function uploadFiles(fileList, bucketName, localDir, bucketPrefix, followSymlinks, client, cli) {
  return Promise.all(
    fileList.map((file) => uploadFile(file, bucketName, localDir, bucketPrefix, followSymlinks, client, cli))
  )
}
...
class ServerlessS3LocalSync {
  ...
  sync() {
    ...
    const promises = this.s3Sync.map((s) => {
      ...
      cli.consoleLog(
        `${messagePrefix} Searching for files with pattern ${localDirGlob} to sync to bucket ${s.bucketName}.`
      )
      return getFileList(localDirGlob, { nodir: true }).then((fileList) => {
        cli.consoleLog(`${messagePrefix} Files found: ${JSON.stringify(fileList, null, 2)}`)
        return uploadFiles(fileList, s.bucketName, s.localDir, s.bucketPrefix, s.followSymlinks, this.client, cli)
      })
    })
